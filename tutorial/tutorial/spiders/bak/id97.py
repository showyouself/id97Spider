...
class DmozSpider(scrapy.Spider):
	...
	l = open('/home/wwwroot/py/tutorial/tutorial/spiders/result/' + str(count) + '-' + str(max_count) + 'lost.log', 'a')
	r = open('/home/wwwroot/py/tutorial/tutorial/spiders/result/' + str(count) + '-' + str( max_count) + 'result.json','a')
	data = {}	
	def go(self):
		self.count = self.count + 1
		url = "http://www.id97.com/movie/" + str(self.count) +".html" 
	...
	def parse(self, response):
		...
		if response.status in self.handle_httpstatus_list:
			tmp = {'id': str(self.count),'status': str(response.status)}
			line = json.dumps(dict(tmp)) + "\n"
			self.l.write(line)
			return self.go()
		item = {'id':self.count,'name':'','year':'','logo_url':'','director':'','screenwriter':'','tostar':'','type':'','country':'','language':'','releasetime':'','long':'','altname':'','score':'','tags':[],'introduce':[],'screenshot':[],'download':{}}
		...
		if tmp > 0:
			print "===================== 解析tags"
			item['tags'] = self.p(response.css('.tags a'))
	...
	def go_ed2k(self):
		url = "http://www.id97.com/videos/resList/" + str(self.count)
		print "================================== 资源请求：", url
	...
	def ed2k_parse(self, response):
		...
		ser = '.text-break a[mid="' +str(self.count) + '"]'
		if tmp > 0:
